...
impl Debugger {
    pub fn new() -> Debugger {
        ...
        for y in 1..26 {
            for x in 0..40 {
                dbg.font.draw_char_rgb(
                    &mut dbg.window_buffer,
                    DEBUG_W,
                    8 * x as usize,
                    8 + 8 * y as usize,
                    102,
    ...
    pub fn render(&mut self, cpu: &mut c64::cpu::CPUShared, memory: &mut c64::memory::MemShared) {
        if self.debug_window.is_open() {
            ...
            let _ = self
                .debug_window
                .update_with_buffer(&self.window_buffer, DEBUG_W, DEBUG_H);
        }
        if self.vic_window.is_open() {
            let _ = self
                .vic_window
                .update_with_buffer(&self.vic_buffer, DEBUG_W, DEBUG_H);
        }
    }
    ...
    pub fn update_vic_window(&mut self, vic: &mut c64::vic::VICShared) {
        ...
        dst_color = if is_state_changed {
            self.mix_colors(VIC_WRITE_COLOR, dst_color, 0.8)
        } else {
            dst_color
        };
        dst_color = if is_raster_irq {
            self.mix_colors(RASTER_COLOR, dst_color, 0.8)
        } else {
            dst_color
        };
        dst_color = if is_bad_line {
            self.mix_colors(BADLINE_COLOR, dst_color, 0.5)
        } else {
            dst_color
        };

        self.vic_buffer[((x as u32) - 1 + (RASTER_DEBUG_W as u32) * (y as u32)) as usize] =
            dst_color;
    }
    ...
    fn draw_ram(&mut self, memory: &mut c64::memory::MemShared) {
        if self
            .debug_window
            .is_key_pressed(Key::PageUp, KeyRepeat::Yes)
        {
        ...
        if self
            .debug_window
            .is_key_pressed(Key::PageDown, KeyRepeat::Yes)
        {
        ...
        self.font
            .draw_text(&mut self.window_buffer, DEBUG_W, 34, 0, "*RAM*", 0x0E);

        for y in 0..26 {
            for x in 0..40 {
                let byte = memory
                    .borrow_mut()
                    .get_ram_bank(c64::memory::MemType::Ram)
                    .read(start);
                self.font.draw_char(
                    &mut self.window_buffer,
                    DEBUG_W,
                ...
                if start == (self.mempage_offset as u16 + 0x0400) {
                    return;
                }
            }
            hex_offset_x = 0;
    ...
    fn draw_vic(&mut self, memory: &mut c64::memory::MemShared) {
        ...
        self.font
            .draw_text(&mut self.window_buffer, DEBUG_W, 34, 0, "*VIC*", 0x0E);

        for y in 0..25 {
            for x in 0..40 {
                let byte = memory
                    .borrow_mut()
                    .get_ram_bank(c64::memory::MemType::Io)
                    .read(start);
                self.font.draw_char(
                    &mut self.window_buffer,
                    DEBUG_W,
    ...
    fn draw_cia(&mut self, cpu: &mut c64::cpu::CPUShared) {
        ...
        self.font
            .draw_text(&mut self.window_buffer, DEBUG_W, 34, 0, "*CIA*", 0x0E);
    ...
    fn draw_sid(&mut self, memory: &mut c64::memory::MemShared) {
        ...
        self.font
            .draw_text(&mut self.window_buffer, DEBUG_W, 34, 0, "*SID*", 0x0E);
    ...
    fn draw_color_ram(&mut self, memory: &mut c64::memory::MemShared) {
        ...
        self.font
            .draw_text(&mut self.window_buffer, DEBUG_W, 28, 0, "*COLOR RAM*", 0x0E);
    ...
    fn draw_data(&mut self, memory: &mut c64::memory::MemShared) {
        ...
        let _ = write!(
            &mut vmatrix_txt,
            "${:04X}",
            0x400 * ((d018 >> 4) & 0xF) as u16
        );
        let _ = write!(
            &mut char_txt,
            "${:04X}",
            0x800 * ((d018 >> 1) & 0x07) as u16
        );
        let _ = write!(
            &mut bmp_txt,
            "${:04X}",
            0x2000 * ((d018 >> 3) & 0x01) as u16
        );
        let _ = write!(
            &mut bank_txt,
            "${:04X}",
            0xC000 - 0x4000 * (dd00 & 0x03) as u16
        );
        self.font
            .draw_text(&mut self.window_buffer, DEBUG_W, 43, 3, "Screen: ", 0x0F);
        self.font.draw_text(
            &mut self.window_buffer,
            DEBUG_W,
            51,
        ...
        self.font
            .draw_text(&mut self.window_buffer, DEBUG_W, 45, 4, "Char: ", 0x0F);
        self.font.draw_text(
            &mut self.window_buffer,
            DEBUG_W,
            51,
        ...
        self.font
            .draw_text(&mut self.window_buffer, DEBUG_W, 43, 5, "Bitmap: ", 0x0F);
        self.font.draw_text(
            &mut self.window_buffer,
            DEBUG_W,
            51,
        ...
        self.font
            .draw_text(&mut self.window_buffer, DEBUG_W, 41, 6, "VIC Bank: ", 0x0F);
        self.font.draw_text(
            &mut self.window_buffer,
            DEBUG_W,
            51,
    ...
    fn draw_gfx_mode(&mut self, memory: &mut c64::memory::MemShared) {
        ...
        self.font.draw_text(
            ...
            1,
            "CHR",
            if !bmp_on & !ecm_on { 0x0A } else { 0x0B },
        );
        self.font.draw_text(
            &mut self.window_buffer,
            DEBUG_W,
    ...
    fn draw_cpu(&mut self, cpu: &mut c64::cpu::CPUShared) {
        ...
        self.font
            .draw_text(&mut self.window_buffer, DEBUG_W, 44, 22, "PC:", 0x0F);
        self.font.draw_text(
            &mut self.window_buffer,
            DEBUG_W,
            47,
        ...
        self.font
            .draw_text(&mut self.window_buffer, DEBUG_W, 53, 22, "A:", 0x0F);
        self.font.draw_text(
            &mut self.window_buffer,
            DEBUG_W,
            55,
        ...
        self.font
            .draw_text(&mut self.window_buffer, DEBUG_W, 59, 22, "X:", 0x0F);
        self.font.draw_text(
            &mut self.window_buffer,
            DEBUG_W,
            61,
        ...
        self.font
            .draw_text(&mut self.window_buffer, DEBUG_W, 65, 22, "Y:", 0x0F);
        self.font.draw_text(
            &mut self.window_buffer,
            DEBUG_W,
            67,
        ...
        self.font
            .draw_text(&mut self.window_buffer, DEBUG_W, 71, 22, "SP:", 0x0F);
        self.font.draw_text(
            &mut self.window_buffer,
            DEBUG_W,
            74,
        ...
        self.font
            .draw_text(&mut self.window_buffer, DEBUG_W, 51, 23, "NV-BDIZC:", 0x0F);
        self.font.draw_text(
            &mut self.window_buffer,
            DEBUG_W,
            61,
    ...
    fn draw_border(&mut self) {
        ...
        self.font
            .draw_char(&mut self.window_buffer, DEBUG_W, 8 * 40, 0, 114, 0x0B);
        self.font
            .draw_char(&mut self.window_buffer, DEBUG_W, 8 * 40, 8 * 27, 113, 0x0B);
    }
    ...
    fn draw_vic_window_txt(&mut self) {
        self.font.draw_text_rgb(
            ...
            3,
            "*VIC events*",
            utils::fetch_c64_color_rgba(0x0F),
        );
        self.font.draw_text_rgb(
            &mut self.vic_buffer,
            RASTER_DEBUG_W,
    ...
    fn clear_char(&mut self, x_pos: usize, y_pos: usize) {
        self.font
            .draw_text(&mut self.window_buffer, DEBUG_W, x_pos, y_pos, " ", 0x00);
    }
